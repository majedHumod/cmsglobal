                        ...
                        @foreach($notes as $note)
                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                                <div class="p-6">
                                    ...
                                    @if(auth()->user()->hasRole('admin') || $note->user_id === auth()->id())
                                        <div class="flex space-x-2">
                                            <a href="{{ route('notes.edit', $note) }}" class="flex-1 text-center bg-indigo-600 hover:bg-indigo-700 text-white text-sm py-2 px-3 rounded-md transition-colors">
                                                تعديل
                                            </a>
                                            <form action="{{ route('notes.destroy', $note) }}" method="POST" class="flex-1">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white text-sm py-2 px-3 rounded-md transition-colors" onclick="return confirm('هل أنت متأكد من حذف هذه الملاحظة؟')">
                                                    حذف
                                                </button>
                                            </form>
                                        </div>
                                ...
                                @foreach($myNotes as $note)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('notes.edit', $note) }}" class="text-indigo-600 hover:text-indigo-900">تعديل</a>
                                                <form action="{{ route('notes.destroy', $note) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('هل أنت متأكد من حذف هذه الملاحظة؟')">
                                                        حذف
                                                    </button>
                                                </form>
                                            </div>
