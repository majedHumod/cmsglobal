<footer class="bg-white border-t border-gray-200">
    <div class="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            <!-- Logo and About - Debug Info -->
            <div class="col-span-1 md:col-span-1">
                <div class="flex items-center">
                    @php
                        $siteName = config('app.name', 'Laravel');
                        $siteLogo = null;
                        $siteDescription = 'نظام إدارة محتوى متكامل يوفر حلول متقدمة لإدارة المحتوى الرقمي.';
                        
                        if (isset($siteSettings) && is_array($siteSettings) && isset($siteSettings['general'])) {
                            if (isset($siteSettings['general']['site_name'])) {
                                $siteName = $siteSettings['general']['site_name'];
                            }
                            if (isset($siteSettings['general']['site_logo'])) {
                                $siteLogo = $siteSettings['general']['site_logo'];
                            }
                            if (isset($siteSettings['general']['site_description'])) {
                                $siteDescription = $siteSettings['general']['site_description'];
                            }
                        }
                    @endphp
            ...
            <div class="col-span-1">
                ...
                <ul class="mt-4 space-y-4">                    
                    @php
                        $contactPhone = $contactWhatsapp = $contactEmail = $contactAddress = null;
                        
                        if (isset($siteSettings) && is_array($siteSettings) && isset($siteSettings['contact'])) {
                            $contactPhone = $siteSettings['contact']['contact_phone'] ?? null;
                            $contactWhatsapp = $siteSettings['contact']['contact_whatsapp'] ?? null;
                            $contactEmail = $siteSettings['contact']['contact_email'] ?? null;
                            $contactAddress = $siteSettings['contact']['contact_address'] ?? null;
                        }
                    @endphp
            ...
            <div class="col-span-1">
                ...
                <div class="mt-4 space-y-4">                    
                    @php
                        $appAndroid = $appIos = null;
                        $socialFacebook = $socialTwitter = $socialInstagram = $socialYoutube = null;
                        $footerText = '© ' . date('Y') . ' ' . $siteName . '. جميع الحقوق محفوظة.';
                        
                        if (isset($siteSettings) && is_array($siteSettings)) {
                            if (isset($siteSettings['app'])) {
                                $appAndroid = $siteSettings['app']['app_android'] ?? null;
                                $appIos = $siteSettings['app']['app_ios'] ?? null;
                            }
                            
                            if (isset($siteSettings['social'])) {
                                $socialFacebook = $siteSettings['social']['social_facebook'] ?? null;
                                $socialTwitter = $siteSettings['social']['social_twitter'] ?? null;
                                $socialInstagram = $siteSettings['social']['social_instagram'] ?? null;
                                $socialYoutube = $siteSettings['social']['social_youtube'] ?? null;
                            }
                            
                            if (isset($siteSettings['general']['footer_text'])) {
                                $footerText = $siteSettings['general']['footer_text'];
                            }
                        }
                    @endphp
